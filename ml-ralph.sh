...
if [ -f "$PRD_FILE" ] && [ -f "$LAST_BRANCH_FILE" ]; then
  ...
  if [ -n "$CURRENT_BRANCH" ] && [ -n "$LAST_BRANCH" ] && [ "$CURRENT_BRANCH" != "$LAST_BRANCH" ]; then
    ...
    DATE=$(date +%Y-%m-%d)
    # Strip common prefixes from branch name for folder
    FOLDER_NAME=$(echo "$LAST_BRANCH" | sed -E 's|^(ml-ralph/|ralph/)||')
...
echo ""
echo "ML-Ralph reached max iterations ($MAX_ITERATIONS) without completing all tasks."
